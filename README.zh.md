...
>
> *此前：[llmfit 1.0 — 让每个数字都可验证的里程碑版本 →](https://github.com/GangNiaga/llmfit/discussions/708)*
...
> - [llmserve](https://github.com/GangNiaga/llmserve) — 一个简单的 TUI，用于服务本地 LLM 模型。选择模型、选择后端、开始服务。
> - [llama-panel](https://github.com/GangNiaga/llama-panel) — 用于管理本地 llama-server 实例的原生 macOS 应用。
...
按 `A` 打开高级配置弹窗。该面板允许你调整 TPS 估算、运行模式惩罚和综合评分背后的参数 — 解决部分模型（如 Qwen3 30B）tok/s 被高估的 [问题 #449](https://github.com/GangNiaga/llmfit/issues/449)。
...
签名在[发布流水线](.github/workflows/release.yml)中自动完成：只有由 GitHub Actions 从本仓库构建的产物才会提交签名，签名请求由项目维护者（[@GangNiaga](https://github.com/GangNiaga)）审批。
