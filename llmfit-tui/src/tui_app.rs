...
pub struct App {
    ...
    // Agents data
    pub agents_data: Option<String>,
    pub agents_rx: Option<mpsc::Receiver<String>>,
}
...
impl App {
    ...
    pub fn fetch_agents(&mut self) {
        if self.agents_rx.is_some() || self.agents_data.is_some() {
            return;
        }
        let (tx, rx) = mpsc::channel();
        self.agents_rx = Some(rx);
        thread::spawn(move || {
            let res = ureq::get("http://127.0.0.1:9421/api/agents")
                .timeout(std::time::Duration::from_secs(5))
                .call();
            let data = match res {
                Ok(response) => response.into_string().unwrap_or_else(|_| "Error reading response".to_string()),
