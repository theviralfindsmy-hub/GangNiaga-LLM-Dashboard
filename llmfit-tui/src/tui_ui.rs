...
use crate::tui_app::{
    AdvConfigField, App, AppTab, AvailabilityFilter, BenchOfferState, BenchViewMode, DL_DOCKER,
...
pub fn draw(frame: &mut Frame, app: &mut App) {
    ...
    let tab_titles = vec![
        Line::from(" [F1] Local Fit "),
        Line::from(" [F2] Agents "),
        Line::from(" [F3] Cloud Bench "),
        Line::from(" [F4] Power Tools "),
    ];
    ...
    let tabs = Tabs::new(tab_titles)
        ...
        .select(selected_tab)
        .highlight_style(Style::default().fg(tc.accent).add_modifier(Modifier::BOLD))
        .style(Style::default().fg(tc.muted));
    frame.render_widget(tabs, outer[0]);

    draw_system_bar(frame, app, outer[1], &tc);
    draw_search_and_filters(frame, app, outer[2], &tc);

    if app.show_bench {
        draw_bench(frame, app, outer[3], &tc);
    } else if app.show_benchmarks {
        draw_benchmarks(frame, app, outer[3], &tc);
    } else if app.show_downloads {
        draw_downloads(frame, app, outer[3], &tc);
    } else if app.show_plan {
        draw_plan(frame, app, outer[3], &tc);
    } else if app.show_multi_compare {
        draw_multi_compare(frame, app, outer[3], &tc);
    } else if app.show_compare {
        draw_compare(frame, app, outer[3], &tc);
    } else if app.show_detail {
        draw_detail(frame, app, outer[3], &tc);
    } else if app.active_tab == AppTab::Agents {
        draw_agents(frame, app, outer[3], &tc);
    } else {
...
fn draw_agents(frame: &mut Frame, app: &App, area: Rect, tc: &ThemeColors) {
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(tc.border))
    ...
    let content = match &app.agents_data {
        Some(data) => {
            match serde_json::from_str::<serde_json::Value>(data) {
                Ok(json) => serde_json::to_string_pretty(&json).unwrap_or_else(|_| data.clone()),
                Err(_) => data.clone(),
            }
        },
        None => {
            if app.agents_rx.is_some() {
                "Loading agents data from Aggregator daemon...".to_string()
            } else {
                "No data fetched. Press F2 to retry.".to_string()
            }
        }
    };
