...
<p align="center">
  <a href="https://github.com/GangNiaga/llmfit/actions/workflows/ci.yml"><img src="https://github.com/GangNiaga/llmfit/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
...
>
> *Previously: [llmfit 1.0 — the release where the numbers became verifiable →](https://github.com/GangNiaga/llmfit/discussions/708)*
...
> - [llmserve](https://github.com/GangNiaga/llmserve) — a simple TUI for serving local LLM models. Pick a model, pick a backend, serve it.
> - [llama-panel](https://github.com/GangNiaga/llama-panel) — a native macOS app for managing local llama-server instances.
...
```sh
brew install GangNiaga/llmfit/llmfit
```
...
### Docker / Podman
```sh
docker run ghcr.io/gangniaga/llmfit
```
This prints JSON from `llmfit recommend` command. The JSON could be further queried with `jq`.
```
podman run ghcr.io/gangniaga/llmfit recommend --use-case coding | jq '.models[].name'
```
To launch the interactive TUI instead, pass the global `--tui` flag:
```sh
docker run --rm -it ghcr.io/gangniaga/llmfit --tui
```

### From source
```sh
git clone https://github.com/GangNiaga/llmfit.git
cd llmfit
cargo build --release
...
Signing happens automatically in the [release pipeline](.github/workflows/release.yml): only artifacts built by GitHub Actions from this repository are submitted for signing, and signing requests are approved by the project maintainer ([@GangNiaga](https://github.com/GangNiaga)).
